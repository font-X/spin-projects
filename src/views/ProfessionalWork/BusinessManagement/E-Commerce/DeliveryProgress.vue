  ...
  <el-container id="mainCard">
    ...
    >
      ...
      <span slot="footer" class="dialog-footer">
        <span>
          <el-button type="primary" @click="finished" class="chooseEnter"
            >确 定</el-button
          >
...
export default {
  ...
  components: { PrintDialog, addDeliverySchedule, JyDialog },
  data() {
    return {
      // 打印组件参数开始
      TempType: '0160',
  ...
  methods: {
    ...
    exportExcel() {
      ...
      if (data.length !== 1) {
        ...
        // 获取当前数据明细列表
        this.$api.delivery
          .GetProductListProgress({ containerId: data[0].ContainerId })
          .then((res) => {
            if (res.statusCode !== 200) {
              this.$message({
                message: res.message,
              ...
              res.data.forEach((item, index) => {
                ...
                obj['序号'] = index + 1
                obj['状态'] = data[0]['状态']
                obj['计划单号'] = data[0]['计划单号']
                obj['站点'] = data[0]['站点']
                obj['出货日期'] = data[0]['出货日期']
                obj['柜号'] = data[0]['柜号']
                obj['柜型'] = data[0]['柜型']
                obj['颜色标'] = data[0]['颜色标']
                obj['备注'] = data[0]['备注']
                obj['产品号'] = item['ProductNo']
                obj['产品名称'] = item['chnName']
                obj['计划箱数'] = item['PlanNumber']
                obj['已配箱数'] = item['PickNumber']
                obj['已配包数'] = item['PickPackage']
    ...
    querySearch(queryString, cb) {
      var invoiceAccountHistory = this.getInvoiceAccountHistory('dp-cNumber')
    ...
    querySearch2(queryString, cb) {
      var invoiceAccountHistory = this.getInvoiceAccountHistory('dp-UserCode')
      var results = queryString
        ? invoiceAccountHistory.filter(this.createFilter(queryString))
        : invoiceAccountHistory
      if (results === undefined) {
    ...
    search() {
      ...
      // 查询时存放历史查询记录
      this.handleSearchHistory('dp-cNumber', this.mainTableParams.cNumber)
      // 计划单号
      this.handleSearchHistory(
        'dp-PickingCode',
        this.mainTableParams.PickingCode
      )
      this.handleSearchHistory('dp-UserCode', this.mainTableParams.UserCode)
