  ...
  methods: {
    ...
    GetCDocumentList() {
      ...
      if (this.transportType == 1) {
        this.QueryList.StateChNm = '中国'
      }
      if (this.transportType == 3) {
        this.QueryList.StateChNm = this.ObjectiveCountry
      }
      this.$api.CustomsDeclaration.TransportListQuery({ model: this.QueryList })
        .then((res) => {
