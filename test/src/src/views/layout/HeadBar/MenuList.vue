<template>
    <ul class="jy-menu-list" >
        <li class="jy-menu-item-first" :class="getState(menu.MenuCode)" v-for="menu of pMune" :key="menu.MenuCode" @mouseenter="modifyState(menu.MenuCode,true)" @mouseleave="modifyState()" >
            <span class="jy-menu-text">{{menu.Name}}</span>
            <i class="fa fa-angle-down" aria-hidden="true"></i>
            <ul class="jy-popup-menu" >
                <li class="jy-popup-item" v-for="sMenu of getSMenu(menu.MenuCode,1)" :key="sMenu.MenuCode" >
                    <span>{{sMenu.MenuName}}</span>
                    <el-row :gutter="0">
                        <el-col :span="6" v-for="thMenu of getSMenu(sMenu.MenuCode,2)" :key="thMenu.MenuCode"><div @click="addMenu(thMenu)" class="grid-content">{{thMenu.MenuName}}</div></el-col>                       
                    </el-row>
                </li>
            </ul>
        </li>
        ...
        getSMenu(d,level){
            ...
            if(this.menudata){
                ...
                for(let i = 0; i < dt2.length; i++){
                    ...
                    }else if(level == 1 && d.split(",").indexOf(item.MenuCode.replace(/\s+$/,"")) > -1){
                        sMenu.push(item);
                    }              
                }
