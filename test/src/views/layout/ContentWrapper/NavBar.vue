<template>
    <div>
        <nav class="jy-nav-list" @click="setActiveScreenPageIndex()"> 
            <div class="jy-tabs-wrapper">
                <div class="jy-tabs-scroll" :style="{marginRight: funWidth + 'px'}">  
                    <ul class="jy-tabs" ref="jytabs">
                        <li class="jy-tab" @click="modifyNav(nav)" :class="{active:isActivenav(nav,index),closing:closingNav.MenuCode === nav.MenuCode}" v-for="(nav,index) of navTree" :key="nav.MenuCode">
            ...
            <div class="jy-nav-fun" :style="{width: funWidth + 'px',marginLeft: -funWidth + 'px'}">
                <i @click="showAllMenu()">菜单</i>
                <i v-if="isActivePage">刷新</i>
                <i v-if="isActivePage" @click="toggleScreenState()">全屏</i>
                <i v-if="isActivePage" >查找</i>
                <i v-if="isActivePage" @click="addScreenPage">分屏</i>
                <i v-if="isActivePage" >设置</i>
                <div class="jy-nav-fun-extend" style="display:none">
                    <el-select size="mini" :style="{height:'100px'}" v-model="value" filterable placeholder="请选择">
                        <el-option
                        size="mini"
...
export default {
    ...
    updated(){
        ...
        &&  setTimeout(function(){children[activeIndex].scrollIntoView(true)},200);
    ...
    methods:{
        ...
        toggleScreenState(){
            (this.$store.state.app.isFullScreen ? this.exitFullscreen: this.fullScreen)()
        },
    ...
    .jy-tabs-wrapper{
        ...
        .jy-tabs{
            ...
            .jy-tab.active{
                ...
                box-shadow:1px 0 3px rgba(174,174,174,.35), inset 0 2px 0 #206ca4;
            }
            .jy-tab.closing{
                animation: hidnav .2s;
            }
